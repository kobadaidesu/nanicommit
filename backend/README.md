...
### 既知の制約（PoC として割り切っているもの）

- 問題生成を待つ間（最大30秒）も DB 接続を1本使ったままです。接続プールは10本なので、同時に10件を超える生成が走ると、それ以降のリクエストは5秒待って 503 になります。デモの規模なら問題ありません。
- リクエスト全体のサイズ上限（4MiB）は `Content-Length` ヘッダだけで判定しています。commit の中身（ファイル数・差分）は別途 413 で判定しています。

## Bさん向け：差し込み方

### 0. Aさん担当の補助ファイルについて

design.md 8.5 のディレクトリ構成のうち、次の4つは Aさん担当の補助ファイルです。どれも MCP の処理は含みません。**Bさんが中身を書き換える必要はありません**（使うだけ）。

```
app/
├── config.py            # A：環境変数の読み込み
├── routers/
│   └── common.py        # A：ルーター共通の小さな関数
└── learning/
